package boletin_1_variables_y_condicionales13;
...
/**
 * This program has a issue with numbers that ends with 0, for example: 10, 20,
 * 30, 40, ..., 500, 600, 700, ..., 8000, 9000.
 * In this case, the inverse number will lose the 0 at the beginning, so the
 * inverse number will have less digits than the original number.
 * For example:
 ...
 * This issue can be solved by treating the number as a string, but for now, we
 * will keep it as an integer to keep the code simple.
 * 
 * Another solution could be to use arrays to store each digit, and then reverse
 * the array to get the inverse number.
 * This would also solve the issue with leading zeros.
 * 
 * -------> And, a even simpler solution would be to just compare the digits
 * directly without forming the inverse number (check folder 13_1)
 */
    ...
    public static void main(String[] args) {
        ...
        if (numero >= 0 & numero <= 9999) {
            ...
            if (numero > 999) {
                ...
                System.out.println("\nThe inverse number is: " + inverse_numero + " "
                        + ((Object) inverse_numero).getClass().getName());

                if (numero == inverse_numero) {
                    System.out.println(ConsoleColors.success("\nThe number " + numero + " is a palindrome." + " "
                            + numero + " = " + inverse_numero + "\n"));
                } else {
                    System.out.println(ConsoleColors.error("\nThe number " + numero + " is not a palindrome." + " "
                            + numero + " != " + inverse_numero + "\n"));
                }
                ;
            }
